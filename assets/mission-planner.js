...
12]});}
  ...
  async function recordSafeDemonstration(){
    ...
    const oldStyle=mapEl.getAttribute('style')||'',ph=document.createElement('div'),parent=mapEl.parentNode,next=mapEl.nextSibling;
    ph.style.cssText='width:'+mapEl.offsetWidth+'px;height:'+mapEl.offsetHeight+'px';parent.insertBefore(ph,next);
    document.body.appendChild(mapEl);mapEl.style.cssText='position:fixed!important;inset:0!important;width:100vw!important;height:100vh!important;z-index:2147483000!important;border:0!important;border-radius:0!important;';
    document.documentElement.classList.add('mp-recording-map');setTimeout(()=>state.map.invalidateSize(),100);
    let stream;try{stream=await navigator.mediaDevices.getDisplayMedia({video:{frameRate:30},audio:false,preferCurrentTab:true});}catch{mapEl.setAttribute('style',oldStyle);parent.insertBefore(mapEl,ph);ph.remove();document.documentElement.classList.remove('mp-recording-map');state.map.invalidateSize();return;}
    ...
    const restore=()=>{mapEl.setAttribute('style',oldStyle);parent.insertBefore(mapEl,ph);ph.remove();document.documentElement.classList.remove('mp-recording-map');state.map.invalidateSize();renderMap();};
    rec.ondataavailable=e=>{if(e.data?.size)chunks.push(e.data);};
    rec.onstop=()=>{stream.getTracks().forEach(t=>t.stop());const blob=new Blob(chunks,{type:'video/webm'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`high-os-safe-${slugify(m.event||'evento')}-mapa-${Date.now()}.webm`;a.click();setTimeout(()=>URL.revokeObjectURL(url),3000);restore();};
    rec.start(500);let n=0;const next=()=>{if(n>=combos.length){setTimeout(()=>{if(rec.state!=='inactive')rec.stop();},900);return;}playSafeCombination(m,r,combos[n++],next);};next();
  }
