...
- A nova zona deverá ser adicionada à seleção de zonas do evento "${eventName}".`:'';
    ...
    const safeRouteText=category==='gas'&&safeRoute?(()=>{
      ...
      lines.push('','VISUAL DA TEMPESTADE DE GÁS:','- Toda a área FORA do círculo atual da SAFE deve receber uma camada roxa opaca/translúcida, simulando visualmente uma tempestade de gás cobrindo o mapa.','- O interior da SAFE deve permanecer limpo, sem a camada roxa: somente a zona segura fica sem gás.','- Esta regra permanece em S1, S2, S3, S4 e no fechamento final. Ao mover ou reduzir a SAFE, a borda da tempestade deve acompanhar continuamente o novo centro e o novo raio.','- No fechamento final até 0 m, a tempestade roxa deve ocupar progressivamente toda a área.');
      lines.push('','HUD / AVISOS:','- Durante redução do raio, exibir: "ATENÇÃO — A SAFE ESTÁ FECHANDO".','- Durante deslocamento do centro, exibir: "ATENÇÃO — A SAFE ESTÁ SE MOVIMENTANDO".','- Quando fechamento e movimento ocorrerem juntos, exibir os dois avisos.','- Fora da SAFE, exibir em destaque: "VOCÊ ESTÁ TOMANDO X DE DANO POR SEGUNDO FORA DA SAFE".','- Ao retornar à área segura, remover imediatamente o aviso de dano.');
      lines.push('','ÁUDIO DA TEMPESTADE / SAFE:','- Se tecnicamente possível, reproduzir som ambiente de tempestade/gás enquanto o player estiver na região afetada, usando como referência de sensação e intensidade a tempestade do Fortnite, porém com áudio próprio/licenciado equivalente.','- Quando a SAFE iniciar movimento/fechamento, reproduzir um alerta sonoro curto usando como referência funcional o aviso de movimentação da tempestade do Fortnite, também com áudio próprio/licenciado equivalente.','- O alerta deve tocar na mudança de fase sem ficar repetindo continuamente; o som ambiente da tempestade pode permanecer em loop enquanto aplicável.');
      return lines.join('\n');
    })():'';
